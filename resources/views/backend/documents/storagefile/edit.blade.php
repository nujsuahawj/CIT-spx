      ...
      <div class="row">
        <div class="col-md-4">
          <div class="card">
            ...
            <form method="POST" action="{{route('storage_file.update', $storage_file->id)}}">
                @csrf
                @method('PATCH')
                <div class="card-body">
                    <div class="form-group">
                      <label for="name">{{__('doc.storage_filename')}}</label>
                      <input type="text" name="name" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" value="{{$storage_file->name}}" placeholder="{{__('doc.storage_filename')}}">
                      @if ($errors->has('name'))
