      ...
      <div class="row">
        <div class="col-md-4">
          <div class="card">
            ...
            <form method="POST" action="{{route('doc_type.update', $doc_type->id)}}">
                @csrf
                @method('PATCH')
                <div class="card-body">
                    <div class="form-group">
                      <label for="name">{{__('doc.doc_typename')}}</label>
                      <input type="text" name="name" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" value="{{$doc_type->name}}" placeholder="{{__('doc.doc_typename')}}">
                      @if ($errors->has('name'))
