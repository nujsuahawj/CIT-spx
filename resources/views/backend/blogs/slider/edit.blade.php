...
<section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>{{__('blog.slider')}}</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">{{__('lang.home')}}</a></li>
            <li class="breadcrumb-item active">{{__('blog.slider')}}</li>
          </ol>
        </div>
      </div>
    </div>
          ...
          <div class="card">
            ...
            <form method="POST" action="{{route('slider.update', $slider->id)}}" enctype="multipart/form-data">
                      ...
                      <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="status">
                                <option value="1" {{$slider->status == 1 ? 'selected' : ''}}>{{ __('blog.active') }}</option>
                                <option value="0" {{$slider->status == 0 ? 'selected' : ''}}>{{ __('blog.inactive') }}</option>
                            </select>
                          </div>
                      </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="title_la" value="{{$slider->title_la}}" class="form-control {{ $errors->has('title_la') ? ' is-invalid' : '' }}" placeholder="{{__('blog.title')}}">
                        ...
                        <div class="col-md-6">
                          <div class="form-group">
                              ...
                              <input type="text" name="title_en" value="{{$slider->title_en}}" class="form-control {{ $errors->has('title_en') ? ' is-invalid' : '' }}" placeholder="{{__('blog.title')}}" >
                    ...
                    <div class="row">
                        <div class="col-md-6">
                          <div class="form-group">
                              ...
                              <input name="des_la" value="{{$slider->des_la}}" class="form-control">
                          </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="des_en">{{__('blog.des')}} ({{__('lang.en')}})</label>
                                <input name="des_en" value="{{$slider->des_en}}" class="form-control">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="card-footer">
                    ...
                    <a class="btn btn-warning" href="{{route('slider.index')}}" data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('lang.back')}}">{{__('lang.back')}}</a>
                </div>
            </form>
