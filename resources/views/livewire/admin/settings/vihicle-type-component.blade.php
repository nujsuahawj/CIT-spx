<div>
    <section class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1>{{__('lang.vihicletype')}}</h1>
            </div>
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">{{__('lang.home')}}</a></li>
                <li class="breadcrumb-item active">{{__('lang.vihicletype')}}</li>
              </ol>
            </div>
          </div>
        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                              <div class="form-group">
                                <label>{{__('lang.vihicletype')}}</label>
                                <input wire:model="name" type="text" class="form-control @error('name') is-invalid @enderror" placeholder="{{__('lang.vihicletype')}}">
                    ...
                    <table class="table table-bordered table-striped">
                      <thead>
                        ...
                        <tr>
                          <td>{{$stt++}}</td>
                          <td>{{$item->name}}</td>
                          <td>@if($item->status==0) <div class="btn btn-danger btn-xs">{{__('lang.disable')}}</div> @else <div class="btn btn-info btn-xs"> {{__('lang.enable')}} </div>  @endif</td>
                          <td>
